...
const unsigned long NTC_TIME_SLICE = 10;
...
void ComInfoSendCyclic() {
 if (Serial.available())
  {
    BTserial.println(currentTemperature);
    targetTemperature = Serial.read();
  }
}
...
void ActLedCyclic() {
  ...
  static unsigned long lastCommsTime = 0;
  unsigned long currentCommsTime = millis();
  if ((currentCommsTime - lastCommsTime) > 10) {
